  ...
  return (
    <div className="bg-white">
      <section className="text-gray-600 body-font  ">
        <h1 className="pt-10 pb-5 font-medium text-center text-3xl ">Mugs</h1>
...
export async function getServerSideProps(context) {
  ...
  let products = await Product.find({ category: "Hoodsies" });
  let hoodies = {};
  ...
  for (let item of products) {
    if (item.title in hoodies) {
      ...
      if (item.availableQty > 0) {
        ...
        // console.warn(hoodies[item.title].size);
      }
    }
  }
  ...
  //   hoodies = JSON.parse(JSON.stringify(item));
  // }
